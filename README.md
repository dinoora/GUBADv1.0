...
## System Architecture
┌─────────────────┐     ┌──────────────────┐     ┌─────────────────┐
│   GEE Cloud     │────▶│  Python Scripts  │────▶│  Google Drive   │
│  (Landsat/S2/   │     │  (RF Classifier)  │     │  (Raw ISA L0)   │
│   S1/VIIRS)     │     │                  │     │                 │
└─────────────────┘     └──────────────────┘     └────────┬────────┘
                                                          │
                              ┌───────────────────────────┘
                              ▼
                    ┌──────────────────┐
                    │  Shell Scripts   │  (batch orchestration)
                    │  (isas_gen.sh)   │
                    └────────┬─────────┘
                             │
                    ┌────────▼─────────┐
                    │   C++ Filter     │  (PAVA isotonic regression)
                    │ (main_IS_filter) │  → L1 temporally consistent ISA
                    └────────┬─────────┘
                             │
                    ┌────────▼─────────┐
                    │  UBA Delineation │  (Python / GIS workflow)
                    │  (morphological  │  → Final GUBAD product
                    │   + population   │    (see Zenodo)
                    │   constraints)   │
                    └──────────────────┘